...
$tracking_number= "TR".strtoupper(substr(md5(microtime(true).mt_Rand()),0,15));
$user_id = $_COOKIE['id'];
$order_status = "completed";

$req = loadDB("SELECT product_id,qty FROM cart WHERE user_id =".$user_id);
...
if (mysqli_num_rows($req) > 0){
    ...
    for ($i=0; $i < count($product_id); $i++) { 
				$rr = loadDB("INSERT INTO orders (order_id,tracking_number,user_id,product_id,qty,order_status) VALUES (NULL,'$tracking_number','$user_id','".$product_id[$i]."','".$qty[$i]."','$order_status')");
				
    }
    loadDB("DELETE FROM cart WHERE user_id =".$user_id);
}
else{
    header('location:index.php'); 
}
...
<div style="text-align:center">
    <h1>Thankyou </h1>
    <hr/>
    <p>Hello <?php echo "<b>".$name."</b>"; ?>,Your payment process is 
    successfully completed <br>and your Transaction id is <b><?php echo $tracking_number; ?></b><br/>
    you can continue your Shopping <br/></p>
    <a href="index.php?c=all" class="btn btn-success btn-lg">Continue Shopping</a>
</div>
